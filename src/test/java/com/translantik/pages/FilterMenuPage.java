...
public class FilterMenuPage extends BasePage {
    ...
    public void methodFilter(String dataColumns) {
        new FilterMenuPage().manageFilterSearchBox.sendKeys(dataColumns);
        BrowserUtils.waitFor(2);
        new FilterMenuPage().getdataColumns(dataColumns).click();
        BrowserUtils.waitFor(2);
        new FilterMenuPage().getColumnsFilterAll(dataColumns).click();
...
//        }
    ...
    public void checkFilteringResults(String filter, String subFilter, Integer int1, Integer int2){
        ...
        List<Integer> columnValues = filterMenuPage.getColumnValues(filter);
        switch (subFilter){
            case "between":
